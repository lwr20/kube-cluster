...
for pod_name, logs in logs_by_pod.iteritems():
    print "Analyzing: %s - %s" % (pod_name, logs)
    pod = pods.get(pod_name)
    try:
        # Format: 2016-04-11T21:04:15Z
        fmt = "%Y-%m-%dT%H:%M:%SZ"
        start_dt = pod["status"]["startTime"]
        start_dt = datetime.datetime.strptime(start_dt, fmt)
...
ordered_start_datetimes = sorted(start_datetimes)
...
# Print out some data.
startup_time = (ordered_start_datetimes[-1] - ordered_start_datetimes[0]).seconds
print "Time to start %s pods: %s" % (len(x), startup_time)
